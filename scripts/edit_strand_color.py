...
device = "cuda"
save_path = "./outputs/edit_strand_color"
radius_scale = 0.5
num_edges = 4
...
for hair_head_dir_path in hair_head_dir_path_list:
    ...
    with torch.no_grad():
        ...
        for edit_mode in ["red", "green", "blue"]:
            os.makedirs(os.path.join(save_path, os.path.basename(hair_head_dir_path), "render_rgb", edit_mode), exist_ok=True)
            for i in tqdm(range(0, 120, 1)):
                ...
                if edit_mode == "red":
                    all_mesh_rgb_tensor[head_mesh.vertices.shape[0]:, 0] += i * 0.005
                    all_mesh_rgb_tensor = torch.clamp(all_mesh_rgb_tensor, max=1, min=0)
                elif edit_mode == "green":
                    all_mesh_rgb_tensor[head_mesh.vertices.shape[0]:, 1] += i * 0.005
                    all_mesh_rgb_tensor = torch.clamp(all_mesh_rgb_tensor, max=1, min=0)
                elif edit_mode == "blue":
                    all_mesh_rgb_tensor[head_mesh.vertices.shape[0]:, 2] += i * 0.005
                    all_mesh_rgb_tensor = torch.clamp(all_mesh_rgb_tensor, max=1, min=0)
                ...
                Image.fromarray(rgb_img).save(os.path.join(save_path, os.path.basename(hair_head_dir_path), "render_rgb", f"{edit_mode}", f"{name}.png"))

            rgb_img_files = sorted(glob(os.path.join(f"{save_path}/{os.path.basename(hair_head_dir_path)}/render_rgb/{edit_mode}", '*.png')))
            rgb_imgs = [cv2.imread(f) for f in rgb_img_files]
            rgb_imgs = [cv2.cvtColor(i, cv2.COLOR_BGR2RGB) for i in rgb_imgs]
            imageio.mimsave(f"{save_path}/{os.path.basename(hair_head_dir_path)}/rgb_render_{edit_mode}.mp4", rgb_imgs, fps=30)
