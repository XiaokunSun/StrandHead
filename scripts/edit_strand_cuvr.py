...
device = "cuda"
save_path = "./outputs/edit_strand_curv"
radius_scale = 0.5
num_edges = 4
...
for hair_head_dir_path in hair_head_dir_path_list:
    ...
    with torch.no_grad():
        ...
        ori_norm = np.sqrt((ori ** 2).sum(-1))
        ori = ori / (ori_norm[:, :, None] + 1e-20)
        curv = ori[:, 1:, :] - ori[:, :-1, :]
        curv_norm = np.sqrt((curv ** 2).sum(-1))
        curv = curv / (curv_norm[:, :, None] + 1e-20)
        
        for i in tqdm(range(0, 120, 1)):
            new_curv_norm = curv_norm * (0.5 + (i / 120))
            tmp_strand_ori = np.concatenate((ori[:, 0:1, :], new_curv_norm[:, :, None] * curv), axis=1)
            tmp_all_strand_ori = np.cumsum(tmp_strand_ori, axis=1)
            tmp_all_strand_ori = tmp_all_strand_ori / (np.sqrt((tmp_all_strand_ori ** 2).sum(-1))[:, :, None] + 1e-20)
            tmp_strand = np.concatenate((new_strand[:, 0:1, :], ori_norm[:, :, None] * tmp_all_strand_ori), axis=1)
