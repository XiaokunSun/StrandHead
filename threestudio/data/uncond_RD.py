...
class RandomCameraDataset(Dataset):
    def __init__(self, cfg: Any, split: str) -> None:
        ...
        elif self.split == "multiview_test":
            self.n_views = self.cfg.n_test_views
        ...
        if self.split == "val":
            ...
            real_batch_size = self.n_views // 4
            azimuth_deg = (
        ...
        self.cx = torch.full_like(focal_length, self.cfg.eval_width / 2)
        self.cy = torch.full_like(focal_length, self.cfg.eval_height / 2)
        identity_c2w: Float[Tensor, "B 3 4"] = torch.eye(4).to(c2w)[None, :3]
        identity_c2w = repeat(identity_c2w, "1 w h -> b w h", b=c2w.shape[0])
        _, rays_d_tmp = get_rays(directions, identity_c2w, keepdim=True)
    ...
    def __getitem__(self, index):
        return {
            ...
            "relative_distances": self.relative_distances[index],
        }
