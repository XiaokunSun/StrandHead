...
class SDSDUControlNetGuidance(BaseObject):
    ...
    def edit_latents(
        ...
        with torch.no_grad():
            ...
            for i, t in enumerate(self.scheduler.timesteps):
                ...
                if ori_text_embeddings is not None:
                    with torch.no_grad():
                        # pred noise
                        ...
                        down_block_res_samples, mid_block_res_sample = self.forward_controlnet(
                            latent_model_input, t, 
                            encoder_hidden_states=ori_text_embeddings,
                            image_cond=image_cond,
                            condition_scale=self.condition_scale
                        )
                        ori_noise_pred = self.forward_control_unet(latent_model_input, t, 
                            encoder_hidden_states=ori_text_embeddings, 
                            cross_attention_kwargs=None, 
                            down_block_additional_residuals=down_block_res_samples, 
                            mid_block_additional_residual=mid_block_res_sample)
                    ...
                    ori_noise_pred_text, noise_pred_uncond = ori_noise_pred.chunk(2)
                    
                    e_old = ori_noise_pred_text - noise_pred_uncond
                    e_new = noise_pred_text - noise_pred_uncond
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if ori_prompt_utils is not None:
            ori_text_embeddings = ori_prompt_utils.get_text_embeddings(elevation, azimuth, camera_distances, human_part, True)
        else:
            ori_text_embeddings = None
        ...
        if self.cfg.use_sds: # SDS loss
            grad = self.compute_grad_sds(text_embeddings, latents, image_cond, t, ori_text_embeddings=ori_text_embeddings)
            ...
            if (self.global_step % self.cfg.per_editing_step == 0) or index not in self.edited_images:
                edit_latents = self.edit_latents(text_embeddings, latents, image_cond, t, ori_text_embeddings=ori_text_embeddings)
