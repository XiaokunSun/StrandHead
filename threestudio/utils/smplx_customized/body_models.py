...
class SMPLX(SMPLH):
    ...
    def forward(
        ...
        **kwargs
        ...
        body_pose = body_pose.to(global_orient.device)  # Ensure that the device is the same
        ...
        if len(kwargs) > 0:
            vertices, joints, A_processed = lbs(shape_components, full_pose, self.v_template,
                                    shapedirs, self.posedirs,
                                    self.J_regressor, self.parents,
                                    self.lbs_weights, pose2rot=pose2rot,
                                    return_affine_mat = True, **kwargs)
        ...
        output = SMPLXOutput(vertices=vertices if return_verts else None,
                             ...
                             v_shaped=v_shaped,
                             full_pose=full_pose if return_full_pose else None,
                             A = A, A_processed=A_processed if len(kwargs) > 0 else None)
        return output
