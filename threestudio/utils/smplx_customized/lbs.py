...
def lbs(
    ...
    **kwargs
    ...
    # Process the vertices according to Gaussian

    if "shape_init_params" in kwargs.keys():
        assert v_shaped.shape[0] == 1
        centroid = v_shaped.mean(0)
        v_shaped = v_shaped - centroid
        v_shaped[:,1] = v_shaped[:,1] + 0.3
        scale = torch.abs(v_shaped).max().detach()
        v_shaped = v_shaped / scale * kwargs["shape_init_params"]
    ...
    if "vertices" in kwargs.keys():
        tmp_J = vertices2joints(J_regressor, kwargs["vertices"])
        _, A = batch_rigid_transform(rot_mats, tmp_J, parents, dtype=dtype)
        # print(A.shape, tmp_J.shape, kwargs["vertices"].shape)
